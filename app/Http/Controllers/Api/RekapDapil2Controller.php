...
class RekapDapil2Controller extends Controller
{
    ...
    {
        $desa = Rekap_desa_dapil2::inRandomOrder()->get();
        ...
        foreach ($sonuos as $index => $data) {
            ...
            $existingDataInDesa = Rekap_desa_dapil2::where('desa', $data->desa)->where('caleg', $data->nm_caleg)->first();

            if (!$existingDataInDesa) {
                Rekap_desa_dapil2::create([
                    'caleg' => $data->nm_caleg,
        ...
        $updatedData = Rekap_desa_dapil2::all();
