...
return new class extends Migration
{
    ...
    {
        Schema::create('ds3__busisingos', function (Blueprint $table) {
            $table->id();
            $table->string('nm_caleg')->constrained('nama_paslon')->on('paslons');
            $table->string('nm_partai')->constrained('nama_partai')->on('paslons');
            $table->string('dapil')->default('DAPIL III');
            $table->string('desa')->default('Busisingo');
            //jumlah suara di tps
    ...
    {
        Schema::dropIfExists('ds3__busisingos');
    }
};
